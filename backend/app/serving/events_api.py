...
def log_impression(request: ImpressionRequest, db: Session = Depends(get_db)) -> dict:
    ...
    db.add(Event(user_id=request.user_id, campaign_id=int(request.ad_id), event_type="impression"))
    db.commit()
    return {"status": "recorded"}


@router.post("/reaction", status_code=201)
def react(request: ReactionRequest, db: Session = Depends(get_db)) -> dict:
    ...
    try:
        record_feedback(db, request.user_id, request.ad_id, request.reaction)
    except ValueError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc

    db.add(Event(user_id=request.user_id, campaign_id=int(request.ad_id), event_type=request.reaction))
    db.commit()
...
def report(request: ReportRequest, db: Session = Depends(get_db)) -> dict:
    ...
    log_event("report_recorded", user_id=request.user_id, ad_id=request.ad_id, category=request.category)
    return {"status": "recorded"}
