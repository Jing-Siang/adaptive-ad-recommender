...
def test_review_campaign_job_approved_activates_campaign(mock_review, db, campaign):
...
@patch("app.campaigns.review_jobs.review_campaign", new_callable=AsyncMock)
def test_review_campaign_job_rejected_sets_status(mock_review, db, campaign):
    campaign.status = "pending_review"
    db.commit()
...
@patch("app.campaigns.review_jobs.review_campaign", new_callable=AsyncMock)
def test_review_campaign_job_needs_review_sets_status(mock_review, db, campaign):
    campaign.status = "pending_review"
    db.commit()
...
@patch("app.campaigns.review_jobs.review_campaign", new_callable=AsyncMock)
def test_review_campaign_job_applies_required_exclusions(mock_review, db, campaign):
    campaign.status = "pending_review"
...
@patch("app.campaigns.review_jobs.review_campaign", new_callable=AsyncMock)
def test_review_campaign_job_missing_campaign_does_not_crash(mock_review):
