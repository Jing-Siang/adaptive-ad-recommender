...
def test_get_performance_totals_reflects_inserted_events(db, campaign):
    ...
    before = client.get("/performance").json()["totals"]
    ...
    try:
        after = client.get("/performance").json()["totals"]
...
def test_get_performance_trend_buckets_by_day(db, campaign):
    ...
    try:
        trend = client.get("/performance").json()["trend"]
...
def test_get_performance_per_campaign_breakdown(db, campaign):
    ...
    try:
        campaigns = client.get("/performance").json()["campaigns"]
...
def test_get_performance_includes_campaigns_with_zero_events(campaign):
